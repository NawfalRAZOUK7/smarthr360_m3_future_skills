    ...
    def test_pagination_applied(self, authenticated_client, db, sample_job_role, sample_skill):
        ...
        for i in range(1, 9):
            skill = Skill.objects.create(
                name=f"Test Skill {i}", category="Technical", description=f"Test skill for pagination {i}"
            )
            skills.append(skill)
        ...
        for i in range(1, 7):
            job_role = JobRole.objects.create(
                name=f"Test Role {i}", department=f"Department {i}", description=f"Test role for pagination {i}"
            )
