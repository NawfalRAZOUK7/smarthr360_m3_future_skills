...
class BulkEmployeeImportSerializer(serializers.Serializer):
    ...
    employees = serializers.ListField(
        child=EmployeeSerializer(), allow_empty=False, help_text="List of employees to import"
    )
    ...
    def validate_employees(self, value):
        ...
        for idx, employee_data in enumerate(value):
            ...
            if email:
                if email in emails:
                    errors.append(
                        {"index": idx, "email": email, "error": f"Duplicate email '{email}' found in import batch"}
                    )
                emails.append(email)
            ...
            if job_role_id:
                try:
                    ...
                    errors.append(
                        {"index": idx, "email": email, "error": f"JobRole with id {job_role_id} does not exist"}
                    )

        if errors:
            raise serializers.ValidationError(
                {"validation_errors": errors, "message": "One or more employees failed validation"}
            )
            ...
            for employee_data in employees_data:
                try:
                    ...
                    if existing_employee:
                        ...
                        created.append({"id": new_employee.id, "email": new_employee.email, "name": new_employee.name})

                except Exception as e:
                    failed.append({"email": employee_data.get("email"), "error": str(e)})
            ...
            for emp_data in all_employees:
                try:
                    ...
                    if employee.job_role:
                        recalculate_predictions(job_role_id=employee.job_role.id, horizon_years=horizon_years)
...
class TrainModelRequestSerializer(serializers.Serializer):
    ...
    dataset_path = serializers.CharField(
        required=False, default="ml/data/future_skills_dataset.csv", help_text="Path to the training dataset CSV file"
    )
    test_split = serializers.FloatField(
        required=False, default=0.2, min_value=0.1, max_value=0.5, help_text="Test set split ratio (0.1 to 0.5)"
    )
    hyperparameters = serializers.JSONField(
        required=False, default=dict, help_text="Model hyperparameters (n_estimators, max_depth, etc.)"
    )
    model_version = serializers.CharField(
        required=False, allow_blank=True, help_text="Version identifier (auto-generated if not provided)"
    )
    notes = serializers.CharField(required=False, allow_blank=True, help_text="Optional notes about this training run")
    async_training = serializers.BooleanField(
        required=False, default=False, help_text="Use Celery for background training (Section 2.5)"
    )
