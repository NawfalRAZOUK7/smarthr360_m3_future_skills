...
def parse_employee_csv(file, encoding: str = "utf-8") -> Tuple[List[Dict[str, Any]], List[Dict[str, str]]]:
    ...
    try:
        ...
        try:
            ...
            errors.append(
                {"row": 0, "field": "encoding", "error": f"File encoding issue detected, used latin-1 fallback"}
            )
        ...
        if missing_headers:
            errors.append(
                {"row": 0, "field": "headers", "error": f'Missing required columns: {", ".join(missing_headers)}'}
            )
            return [], errors
...
def parse_employee_excel(file) -> Tuple[List[Dict[str, Any]], List[Dict[str, str]]]:
    ...
    try:
        ...
        errors.append({"row": 0, "field": "file", "error": f"Failed to parse Excel file: {str(e)}"})
