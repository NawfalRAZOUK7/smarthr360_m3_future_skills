...
class Command(BaseCommand):
    ...
    def _analyze_events(self, events, hours):
        ...
        recent_events = [
            e for e in events
            if self._is_recent(e.get('asctime', ''), cutoff_time)
        ]
        ...
        suspicious = [
            e for e in recent_events
            if e.get('event') == 'suspicious_request'
        ]
        ...
        rate_limited = [
            e for e in recent_events
            if e.get('event') == 'rate_limit_exceeded'
        ]

        # Blocked IPs
        blocked_ips = [
            e for e in recent_events
            if e.get('event') == 'ip_blocked'
        ]
    ...
    def _print_analysis(self, analysis):
        ...
        for severity, count in sorted(analysis['severities'].items()):
            style = self.style.ERROR if severity == 'ERROR' else self.style.WARNING if severity == 'WARNING' else self.style.SUCCESS
            self.stdout.write(style(f"  {severity}: {count}"))

        # Security metrics
        self.stdout.write(self.style.WARNING('\nSecurity Metrics:'))
        self.stdout.write(f"  Failed Authentication Attempts: {analysis['auth_failures']}")
        self.stdout.write(f"  Suspicious Activities: {analysis['suspicious_activities']}")
        self.stdout.write(f"  Rate Limited Requests: {analysis['rate_limited_requests']}")
        ...
        if analysis['recent_suspicious']:
            ...
            for event in analysis['recent_suspicious'][-5:]:
                self.stdout.write(f"  {event.get('asctime', 'N/A')}: {event.get('message', 'N/A')}")
        ...
        if analysis['recent_auth_failures']:
            ...
            for event in analysis['recent_auth_failures'][-5:]:
                self.stdout.write(f"  {event.get('asctime', 'N/A')}: {event.get('username', 'unknown')} from {event.get('ip_address', 'unknown')}")
