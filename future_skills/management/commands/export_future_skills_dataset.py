...
TECHNICAL_KEYWORDS = [
    'python', 'ia', 'data', 'cloud', 'devops', 'machine learning',
    'blockchain', 'cybersecurity', 'kubernetes', 'java', 'javascript',
    'aws', 'azure', 'docker', 'sql', 'nosql'
]
...
def _get_market_trend_for_context(job_role, skill) -> float:
    ...
    if job_role.department:
        trend = MarketTrend.objects.filter(
            sector__icontains=job_role.department
        ).order_by('-year', '-trend_score').first()
        if trend:
    ...
    if skill.category:
        trend = MarketTrend.objects.filter(
            sector__icontains=skill.category
        ).order_by('-year', '-trend_score').first()
        if trend:
            return max(0.0, min(1.0, float(trend.trend_score)))
...
def _get_economic_indicator(job_role) -> float:
    ...
    report = EconomicReport.objects.filter(
        sector__icontains=dept
    ).order_by('-year').first()
