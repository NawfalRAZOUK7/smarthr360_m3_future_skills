...
class CeleryTrainingTest(TestCase):
    ...
    def test_async_training_fallback_on_celery_failure(self):
        ...
        with patch("future_skills.tasks.train_model_task") as mock_task:
            ...
            response = self.client.post(
                "/api/training/train/",
                {"async_training": True, "hyperparameters": {"n_estimators": 20}, "model_version": "test_celery_fail"},
                format="json",
            )
    ...
    def test_training_run_created_before_async_dispatch(self):
        ...
        with patch("future_skills.tasks.train_model_task") as mock_task:
            ...
            response = self.client.post(
                "/api/training/train/",
                {"async_training": True, "hyperparameters": {"n_estimators": 20}, "model_version": "test_run_creation"},
                format="json",
            )
