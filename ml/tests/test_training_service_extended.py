...
class TestDataLoadingEdgeCases:
    ...
    def test_load_data_unexpected_error(self, tmp_path):
        ...
        data = {
            'trend_score': [0.8, 0.6] * 5,
            'future_need_level': ['HIGH', 'LOW'] * 5
        }
        df = pd.DataFrame(data)
        df.to_csv(dataset_path, index=False)
...
class TestTrainingErrorScenarios:
    ...
    def test_train_logs_mlflow_run_id(self, tmp_path, caplog):
        ...
        with patch('future_skills.services.training_service.get_mlflow_config') as mock_config:
            ...
            assert any('test-run-id-12345' in record.message for record in caplog.records)
...
class TestModelPersistenceErrors:
    ...
    def test_save_model_handles_permission_error(self, tmp_path):
        ...
        with patch('future_skills.services.training_service.joblib.dump',
...
class TestFeatureImportanceEdgeCases:
    ...
    def test_get_feature_importance_handles_extraction_error(self, tmp_path):
        ...
        with patch.object(trainer.model.named_steps['clf'], 'feature_importances_',
                         new_callable=PropertyMock) as mock_importance:
    ...
    def test_get_feature_importance_with_feature_count_mismatch(self, tmp_path, caplog):
        ...
        import numpy as np
        with patch.object(trainer.model.named_steps['clf'], 'feature_importances_',
                         new=np.array([0.5, 0.3])):  # Wrong number of features
            with caplog.at_level('WARNING'):
                importance = trainer.get_feature_importance()

            # Should log warning and return empty dict
            assert any('Feature count mismatch' in record.message for record in caplog.records)
            assert importance == {}
...
class TestTrainingRunSaving:
    ...
    def test_save_training_run_handles_mlflow_transition_error(
        self, mock_mlflow, mock_manager_class, mock_create_version, 
        mock_training_run, tmp_path, caplog
    ):
        ...
        assert any('Failed to transition MLflow stage' in record.message 
    ...
    def test_save_training_run_handles_database_error(self, tmp_path):
        ...
        with patch('future_skills.services.training_service.create_model_version',
    ...
    def test_save_failed_training_run_handles_save_error(self, mock_training_run, tmp_path):
        ...
        with pytest.raises(Exception, match="DB error"):
            trainer.save_failed_training_run(
                model_version="v1.0.0",
                error_message="Original training error"
            )
...
class TestMLflowIntegration:
    ...
    def test_train_logs_per_class_metrics_to_mlflow(self, mock_config, tmp_path):
        ...
        with patch('future_skills.services.training_service.mlflow') as mock_mlflow:
            ...
            assert any('LOW_' in str(call) or 'MEDIUM_' in str(call) or 'HIGH_' in str(call)
