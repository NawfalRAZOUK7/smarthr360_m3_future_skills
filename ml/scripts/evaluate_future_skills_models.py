...
def print_metrics_summary(metrics: Dict[str, Any]):
    ...
    logger.info("\n  Confusion Matrix:")
    logger.info(f"  {'-'*66}")
    pred_header = "Actual \\ Pred"  # backslash kept out of the f-string expr (SyntaxError on py<3.12)
    logger.info(f"  {pred_header:<15} {LABEL_ORDER[0]:<10} {LABEL_ORDER[1]:<10} {LABEL_ORDER[2]:<10}")
    logger.info(f"  {'-'*66}")
